...
app.controller("damage_import_controller", ["$scope","$rootScope","$state","$stateParams", "$host", "_config", "_basic",'_socket', function ($scope, $rootScope,$state,$stateParams,$host, _config, _basic,_socket) {
    ...
    var colObjs = [
        ...
        {name: '起始地ID', type: 'number', length: 3, require: true},
        {name: '起始装车地ID', type: 'number', length: 3},
        {name: '目的地ID', type: 'number', length: 3, require: true},
        {name: '经销商ID', type: 'number',length: 3, require: true},
        {name: '质损说明', type: 'string', require: true}];
    // 头部条件判断
    ...
    function  getLocalFile(id){
        _basic.formPost($("#file_upload_form"), $host.api_url + '/user/' + userId + '/damageFile?uploadId='+id , function (data) {
            if (data.success == true) {
