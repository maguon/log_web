 ...
 */
app.controller("storage_car_brand_controller", ["$scope", "$host", "_basic", function ($scope, $host, _basic) {
    ...
    // 修改汽车品牌
    $scope.amend_brand_submit = function (id,name,$index) {
        _basic.put($host.api_url + "/user/" + adminId + "/carMake/" + id, {
            "makeName": name
        }).then(function (data) {
            if (data.success == true) {
                $(".brand_box"+$index).show();
    ...
    // 增加汽车型号接口
    $scope.add_car_model_submit=function (id) {
        _basic.post($host.api_url + "/user/" + adminId + "/carMake/" + id+"/carModel", {
    ...
    $scope.changeSelfBrandStatus = function (id, status, makeId) {
        ...
        _basic.put($host.api_url + "/user/" + adminId + "/carModel/" + id + "/modelStatus/" + status, {}).then(function (data) {
            if (data.success == true) {
    ...
    $scope.amend_car_model_submit = function (id, name,index) {
        _basic.put($host.api_url + "/user/" + adminId + "/carModel/" + id, {
            modelName: name
        }).then(function (data) {
            if (data.success == true) {
    ...
    $scope.amend_car_model_status=function (id,sta,makeId) {
        ...
        _basic.put($host.api_url + "/user/" + adminId + "/carModel/" + id+"/modelStatus/"+sta,{}).then(function (data) {
            if (data.success == true) {
                swal("更改状态", "", "success");
                $scope.search_carModel(makeId);
    ...
    $scope.add_brand=function () {
        _basic.post($host.api_url + "/user/" + adminId + "/carMake/", {
